...
const MedicationCard = React.memo(({ item, onDelete, onEdit, onSetReminder }: MedicationCardProps) => {
  ...
  const handleDelete = () => {
    Alert.alert(
      ...
      [
        ...
        { text: "Delete", style: "destructive", onPress: () => onDelete(item.id, item.notif_id) },
      ]
    );
  };
    ...
    <View style={[styles.card, isDefault && styles.cardDefault]}>
      ...
      <View style={styles.cardRow}>
        ...
        <View style={styles.cardInfo}>
          <Text style={styles.cardBrand} numberOfLines={1}>{item.brand_name}</Text>
          {item.generic_name ? (
            <Text style={styles.cardGeneric} numberOfLines={1}>{item.generic_name}</Text>
          ) : null}
          ...
          {item.dose ? (
            <Text style={styles.cardDose} numberOfLines={1}>
              <Ionicons name="medical-outline" size={12} color="#8E9BAE" /> {item.dose}
            </Text>
          ) : null}
      ...
      <TouchableOpacity style={styles.reminderBtn} onPress={() => onSetReminder(item)}>
        <Ionicons
          name={formattedTime ? "alarm" : "alarm-outline"}
          size={15}
          color={formattedTime ? "#4F8EF7" : "#8E9BAE"}
        />
        <Text style={[styles.reminderBtnText, formattedTime && { color: "#4F8EF7" }]}>
          {formattedTime ? `Reminder: ${formattedTime}  (tap to change)` : "Set Reminder"}
        </Text>
      </TouchableOpacity>
    </View>
  );
  ...
  const handleEdit = (medication: any) => {
    router.push({
      ...
      params: {
        ...
        imageUri:    medication.image_uri && medication.image_uri.trim() !== ""
  ...
  const renderEmpty = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptySubtitle}>
        {query ? "Try a different search term." : 'Tap "Add Medication" to get started.'}
      </Text>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        <Ionicons name="search" size={18} color="#8E9BAE" style={styles.searchIcon} />
        <TextInput
      ...
      {/* List */}
      {loading && !refreshing ? (
        <ActivityIndicator color="#4F8EF7" size="large" style={{ marginTop: 48 }} />
      ) : (
        <FlatList
          data={medications}
...
const styles = StyleSheet.create({
  ...
  searchContainer: {
    ...
    backgroundColor: "#1A2740", margin: 16, marginBottom: 8,
  ...
  countText: { color: "#8E9BAE", fontSize: 12, marginHorizontal: 20, marginBottom: 4 },

  // ── Card ──
  card: {
    backgroundColor: "#1A2740", borderRadius: 16,
  ...
  badge: {
    ...
    backgroundColor: "#0E2044", borderRadius: 20,
    paddingHorizontal: 10, paddingVertical: 4,
    marginBottom: 10, borderWidth: 1, borderColor: "#1E3A6E",
  },
  badgeText: { color: "#4F8EF7", fontSize: 11, fontWeight: "700", letterSpacing: 0.3 },

  cardRow:    { flexDirection: "row", alignItems: "flex-start" },
  cardImage:  { width: 64, height: 64, borderRadius: 12, backgroundColor: "#0A1628", marginRight: 14 },
  cardInfo:   { flex: 1, minWidth: 0 },

  cardBrand:   { color: "#fff", fontSize: 16, fontWeight: "700", marginBottom: 2 },
  ...
  reminderBtn: {
    ...
    borderTopWidth: 1, borderTopColor: "#2D3E55",
  },
  reminderBtnText: { color: "#8E9BAE", fontSize: 13, fontWeight: "600", flex: 1 },

  // ── Empty ──
  emptyState: { flex: 1, alignItems: "center", justifyContent: "center", paddingTop: 80 },
  emptyTitle: { color: "#4A5568", fontSize: 18, fontWeight: "600", marginTop: 16 },
  emptySubtitle: {
    color: "#4A5568", fontSize: 14, marginTop: 8,
